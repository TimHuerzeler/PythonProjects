...
according the grammar given in the Parser_Build class.
(no precedence between the operators. Example:  ((((3+5)-3)*(4+4))/(2*4)))

This version parse an expression, build the expression tree (parse tree) and evaluate the expression.

Note that this version does not take into account neither unary operators nor variables and 
...
class Parser_Builder:
    ...
    def parse_build(self) -> Term:
        """
        Recursive function to parse an arithmetic expression and to build
        ...
        if self.current[0] == Token.CTE:                        # constant ?
            ...
            if self.current[0] == Token.PARR:                   # ) ?
                ...
                if   self.current[0] == Token.ADD: bin_op = Bin_op.ADD; # binary operator?
                elif self.current[0] == Token.SUB: bin_op = Bin_op.SUB;
                elif self.current[0] == Token.MUL: bin_op = Bin_op.MUL;
                elif self.current[0] == Token.DIV: bin_op = Bin_op.DIV;
                else:
                    raise ParsingException("Wrong operator or right parenthesis expected")
                self.current = self.next_token()                # reads next token
                term2 = self.parse_build()                      # recursion for ( ... oper expr )
                if self.current[0] == Token.PARR:               # ) ?
                    bin_exp = Binary_expression(term1, term2, bin_op)
                    self.current = self.next_token()            # reads next token
...
def main() -> None:
    ...
    parser_builder = Parser_Builder(expr)
